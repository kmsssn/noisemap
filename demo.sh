...
# Прогоняет полный цикл по основным endpoints
# ============================================================
...
for port in 8081 8082 8084 8085 8086 8087 8088; do
...
echo -e "  ${GREEN}✅ Токен: ${ACCESS_TOKEN:0:30}...${NC}"
...
DEVICE_CAL=$(curl -s "$USER_SVC/api/v1/devices/calibration?model=Samsung%20Galaxy%20S24" \
  -H "X-User-Id: $USER_ID" \
  -H "Authorization: Bearer $ACCESS_TOKEN")
echo "  Samsung Galaxy S24: $DEVICE_CAL"

DEVICE_CAL2=$(curl -s "$USER_SVC/api/v1/devices/calibration?model=iPhone%2013%20Pro" \
  -H "X-User-Id: $USER_ID" \
  -H "Authorization: Bearer $ACCESS_TOKEN")
echo "  iPhone 13 Pro: $DEVICE_CAL2"
echo ""

# ---- 4. Gamification before ----
...
# ---- 6. Notifications ----
echo -e "${YELLOW}[7/9] Уведомления...${NC}"
...
# ---- 7. Leaderboard ----
echo -e "${YELLOW}[8/9] Лидерборд (топ-5)...${NC}"
LEADERBOARD=$(curl -s "$GAME_SVC/api/v1/gamification/leaderboard?limit=5")
echo "  $LEADERBOARD"
echo ""

# ---- 8. Comments ----
echo -e "${YELLOW}[9/9] Тестовый комментарий...${NC}"
COMMENT=$(curl -s -X POST "$BASE_URL:8088/api/v1/comments" \
  -H "Content-Type: application/json" \
  -H "X-User-Id: $USER_ID" \
  -H "X-Display-Name: Demo User" \
  -d '{
    "latitude": 43.238,
    "longitude": 76.945,
    "text": "Тестовый комментарий с демо-скрипта"
  }')
echo "  $COMMENT"
echo ""
...
echo -e "${CYAN}╠═════════════════════════════════════════════════╣${NC}"
echo -e "${CYAN}║                                                 ║${NC}"
echo -e "${CYAN}║  Swagger UI (через Gateway):                    ║${NC}"
